...
module Player =
  ...
  let init api p playerId =
    ...
    Cmd.batch [ Cmd.OfAsync.either (api.getPlayerInfo playerId) p.Token PlayerReceived (Error >> Init)
                Cmd.OfAsync.either (api.getPlayerPointsTotal playerId) p.Token PlayerPointsTotalReceived (Error >> Init)
                Cmd.OfAsync.either (api.getPlayerFixtureSets playerId) p.Token PlayerFixtureSetsReceived (Error >> Init) ]

  let pointsTotalView points =
    Components.card [ Components.pointsTotalView points ]
         ...
         |> List.map
              (fun (_, m) ->
                ...
                tr [] [
                  td [] [
                    a
                      (Components.anchorNavProps (NavTo >> dispatch) (PlayersRoute(PlayerGameweekRoute(playerId, gwno))))
  ...
  let fullView dispatch (vm: PlayerViewModel) points fs =
    ...
    div [] [
      ...
      (match fs with
       | Some f -> fixtureSetsView dispatch vm f
       | None -> div [] [])
    ]

  let view (model: Model) dispatch =
    match model.Player, model.PointsTotal with
    | Success player, Success points -> fullView dispatch player points model.PlayerFixtureSets
    | _ -> div [] []
  ...
  let update api player msg model : Model * Cmd<Msg> =
    ...
    | PlayerPointsTotalReceived r ->
      ...
      []
    | PlayerFixtureSetsReceived r ->
      { model with
          PlayerFixtureSets = resultToOption r },
      []
